...
@torch.no_grad()
def get_act_stats_falcon(model, dataloader, device_, metric='hessian'):
    nsamples = len(dataloader)
    device = device_
    act_scales = {}
...
@torch.no_grad()
def get_reorder_index(model, act_scales, args):
    act_orders = {}
    ...
    if "llama" in args.model.lower() or "opt" in args.model.lower():
        named_modules = model.model.named_modules()
    elif "falcon" in args.model.lower():
        named_modules = model.transformer.named_modules()
